...
print(f"Št. osnovnih podatkov: {len(osnovni_podatki)}")  # <-- TU

podatki_in_htmlji = pridobi.pridobi_htmlje_avtomobilov(osnovni_podatki)
print(f"Št. prenesenih HTML-jev: {len(podatki_in_htmlji)}")  # <-- TU

avtomobili = izlusci.podrobnosti_modelov(podatki_in_htmlji)
print(f"Št. izluščenih avtomobilov: {len(avtomobili)}")  # <-- TU
